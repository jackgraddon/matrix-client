 ...
 * ─── THE FIX ────────────────────────────────────────────────────────────────
 *
 * We monkey-patch `crypto.subtle.importKey`, `crypto.subtle.deriveKey`,
 * `crypto.subtle.generateKey` and `crypto.subtle.unwrapKey` to silently
 * flip `extractable` to `true` for key types that WebKit would otherwise
 * persist to the Keychain (AES-*, HMAC, Ed25519, etc.).
 *
 ...
 * ensures it only runs in the browser (never during SSR).
 *
 * CRITICAL: The monkey-patch logic is now OUTSIDE defineNuxtPlugin, so it
 * executes the moment this module is evaluated. This ensures it wins the
 * race against any `import 'matrix-js-sdk'` in other files.
 */
...
function applyPatch() {
    ...
    console.log('[webcrypto-patch] Applying patches to crypto.subtle...');
    ...
    // ── Patch unwrapKey ───────────────────────────────────────────────────────
    const _originalUnwrapKey = crypto.subtle.unwrapKey.bind(crypto.subtle);
    // @ts-expect-error
    crypto.subtle.unwrapKey = function patchedUnwrapKey(
        format: Parameters<SubtleCrypto['unwrapKey']>[0],
        wrappedKey: Parameters<SubtleCrypto['unwrapKey']>[1],
        unwrappingKey: Parameters<SubtleCrypto['unwrapKey']>[2],
        unwrapAlgorithm: Parameters<SubtleCrypto['unwrapKey']>[3],
        unwrappedKeyAlgorithm: Parameters<SubtleCrypto['unwrapKey']>[4],
        extractable: boolean,
        keyUsages: Parameters<SubtleCrypto['unwrapKey']>[6],
    ): ReturnType<SubtleCrypto['unwrapKey']> {
        const name = algoName(unwrappedKeyAlgorithm as AlgorithmIdentifier);
        const safeExtractable = NON_EXTRACTABLE_ALGOS.has(name) ? extractable : true;
        return _originalUnwrapKey(format, wrappedKey, unwrappingKey, unwrapAlgorithm, unwrappedKeyAlgorithm, safeExtractable, keyUsages);
    };
...
export default defineNuxtPlugin(() => {
    // Plugin provides nothing, it just ensures the side-effect (applyPatch)
    // is recorded as being part of the Nuxt lifecycle.
});