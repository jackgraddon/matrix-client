...
export default defineEventHandler(async (event): Promise<TauriUpdate | void> => {
  ...
  try {
    ...
    const updateData = await $fetch<TauriUpdate>(`https://tumult.jackg.cc/update.json?t=${Date.now()}`);
